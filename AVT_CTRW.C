...
//---------------------------
float F_Dns_off= 60;       // кг/ч  - расход для проверки закрывания клапана "плотность"
float F_Dns_off_DLV= 120;  // кг/ч  расход для проверки закрывания клапана "плотность" после
                          //        окончания отпуска (4)


float F_Dns_off_DLVs= 120; // кг/ч  -   расход для проверки закрытого сост. клапана "плотность" перед
                          //    открыванием клапанов отсекания дозы (3)


long int T_Dns_off=15000; // мс, допустимое время закрывания клапана "плотность"
...
// MKO 3 4 11 1 5 9 10 16 2 13 12 16    - Молодечно,Казинца

int num_out1=3,num_out2=2;
int num_out3=7,num_out4=1;
int num_out5=0,num_out6=5;
int num_out7=6,num_out8=0;
int num_out9=0,num_out10=0;
int num_out11=0,num_out12=4;

int num_out13=16,num_out14=16;
int num_out15=15,num_out16=16;

int num_in1=4,num_in2=9;
int num_in3=9,num_in4=3;
...
//          давление |
//                 | |   0x60
int analog_num[8]={3,1,2,5,6,0,0,0};  // для 2-х резервуаров
//int analog_num[8]={4,1,2,6,7,3,8,0};  // для 3-х резервуаров
float analog_scale[8]={1.6/MaxADC,1.6/MaxADC,1.6/MaxADC,150./MaxADC,150./MaxADC,
1.6/MaxADC,150/MaxADC,1};


float analog_offset[8]= {0,0,0,-50,-50,0,-50,0};
...
//    Out_off(C_DNS | OUT3);
      f_pmp_OFF();
      f_den_OFF();
      Out_off(OUT1 | OUT2 | OUT5 | OUT8 );
      sw_dns=200;
      Temperature=s_MVD[MVD_DNS].Temp;
      Density=s_MVD[MVD_DNS].Dens;
      f_wr_evt(evt_dn_err);
...
//     Out_off(C_DNS+OUT3);
       ...
       f_MVD_WR((int)MVD_DNS,(int)MVD_WR_F,(int) 195,(int)0,(long int) 0,dens_cutoff);
       sw_dns=-1;
       return 0;
    }
       sw_dns=3;
   ...
   case 3:
    f_set_o67();
 MVD_t_rslt[MVD_DNS]=0;
 MVD_Rqst_T[MVD_DNS].answ_com=f_MVD_rd_t;
 MVD_Rqst_T[MVD_DNS].answ_flt=f_MVD_rd_t;
 time_t_snd=TimeStamp;
 MmiGotoxy(0,1);   MmiPuts("Подготовка расходомера");

      // Stop totalizers
 if( f_MVD_WR((int)MVD_DNS,(int)MVD_WR_C,(int) 2,(int)0,(long int) 0,0) == 0)
...
//-----------------------------
 ...
 //
 time_t_snd=TimeStamp;
 if( f_MVD_WR((int)MVD_DNS,(int)MVD_WR_F,(int) 195,(int)0,(long int) 0,0) == 0)
 { // ошибка при отправке посылки MVD
    goto m_e1;
 }
    sw_dns=42;
...
//-----------------------------
 ...
 //
 time_t_snd=TimeStamp;
 if( f_MVD_WR((int)MVD_DNS,(int)MVD_WR_F,(int) 197,(int)0,(long int) 0,dens_cutoff1) == 0)
 { // ошибка при отправке посылки MVD
    goto m_e1;
 }
    sw_dns=6;
...
//-----------------------------
    case 110:
         ...
         Dens_Result= s_MVD[MVD_DNS].Dens;
         Temp_Result= s_MVD[MVD_DNS].Temp;
//         Out_off(C_DNS);
         f_den_OFF();
         time_t_snd=TimeStamp;
         time_t_NDB=TimeStamp;
...
//-----------------------------
    ...
    if( fabs(s_MVD[MVD_DNS].FlowM) < F_Dns_off) ;
    else
         time_t_NDB=TimeStamp;
...
//         Out_off(C_DNS);
//       Out_off(C_DNS+OUT3);
         Out_off(OUT1 | OUT2 | OUT5 | OUT8 | OUT9 | OUT10 | OUT11  );
         f_set_o67();
         sw_dns=121;
         time_t_snd=TimeStamp;
         ...
         time_t_snd=TimeStamp;
            ...
            // запрещение счета расходомера 1  (cutoff for Mass Flow)
         MVD_t_rslt[MVD_DNS]=0;
         if( f_MVD_WR((int)MVD_DNS,(int)MVD_WR_F,(int) 195,(int)0,(long int) 0,dens_cutoff) == 0)
         { // ошибка при отправке посылки MVD
            goto m_e1;
         }
   break;
...
//-----------------------------
  ...
  if((MVD_fn[MVD_DNS] != 0) && (MVD_fn[MVD_DNS] != F_FAULT))
  {
      if( f_timer(time_t_snd,f_MVD_tim ) )
      {
       goto m_err;
...
/* =========================================
   case  21:
    ...
    {
mfc21:
     // множитель объема расходомера 1
      MVD_t_rslt[0]=0;
      time_t_snd=TimeStamp;
...
//-----------------------------
   case  64:
     ...
     // запись значений Mass_grk, Vol_grk
     ...
     if(f_wrt_eee_kd( EE_grk )  <0 )
     { // ошибка при записи в EEPROM
m_EEP:
        f_icp_errS(EEE_CRC_error);
        MmiGotoxy(0,1);  MmiPuts("! Ошибка EEPROM        ");
        goto m_err;
     }

     if( f_rd_EEE_kd(EE_grk) < 0)
     {
              goto m_EEP;
     }
     if(Mass_grk != ftmp1 )
     ...
     Fl_grk=Fl_grk_FLT;
     if(f_wrt_eee_kd( EE_flg ) <0 )
     {
              goto m_EEP;
     }

     if( f_rd_EEE_kd(EE_flg) < 0)
     {
              goto m_EEP;
     }

     if(Fl_grk != Fl_grk_FLT)
     {
              goto m_EEP;
     }
...
// в ftmp сколько литров осталось
 ...
 if(fl_GO_rcv == 1)
 {
  ...
  TimeOut3=TimeStamp;
        // больший клапан
  if(flag_O1)
    if( (ftmp-k_volr*CL_T1R )> CL_valR[0])
    {
...
/*
  if(sw_dns!=20)
  {
    if ( ( (i_LVL & 0x8000) != 0 ) && ( (INP_VARi & L85) != 0  )  )
      OUT_VAR |= OUT12;
    else
      OUT_VAR &= ~OUT12;
  }
