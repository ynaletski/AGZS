...
int ICP_error[(sizeof(ICP_dd) / sizeof(&MMI))  ]={0,0,0,0,0};
int ICP_error_c[(sizeof(ICP_dd) / sizeof(&MMI))  ]={0,0,0,0,0};
int FL_err=0;
//====================================
 ...
 for(i=1,i1=1;i<icp_lst_max;i++,i1=i1+i1)
 {
   if(ICP_DEV == ICP_dd[i])
   {
     if(error_type)
     {
      Reg_err |= i1;
      if(flag_prn)
         printf("\r\nDevice %d.Err.=%d(%4x)",i,error_type,error_type);
...
int fl_err_new;
 ...
 fl_evt=0;
   if(error_type)
   {
      ...
      if(error_type==WD_pmlan)
      {
       // ошибка зафиксируется только в режиме ГРК
      ...
      if(error_type==ES_off) Reg_err |= ES_off;
      if(error_type==MVD_com) Reg_err |= MVD_com;
      if(error_type==UZA_off)
      {
        if(flagE_UZA==LVL_E )
        {
          Reg_err |= LVL_sb;
          ICP_error[icp_lst_max] |= LVL_sb;
        }
        else// if(flagE_UZA==UZA_E )
        {
          Reg_err |= UZA_off;
          ICP_error[icp_lst_max] |= UZA_off;
        }
      }
      else
