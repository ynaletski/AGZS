...
*/
           ...
           {
           /*======
             if(Cor_ADC[0][0] != MaxADC)
             {
              ftmp=dat_i[itmp];
              ADC[0]=ftmp;
              ftmp+=MaxADC;

              mul_s = ftmp * TT_PER_1_ADC[0];
              j=(int)(mul_s);
              mul_s -= (float)j;
              t_kor=&Cor_ADC[0][j];
              s_MVD[0].Press = t_kor[1] + (t_kor[2] - t_kor[1]) * mul_s;
             }
             else
            ==========*/
      ...
      //     if((Delta[0].status !=0) ||  (ComSK[0].status !=0) )
             if( *DriveStatus !=0)
                if(flag_O3r) f_reg_P(); // регулятор давления
           }
           itmp=analog_num[1]-1;
           if(itmp>= 0)
           {
              ftmp=dat_i[itmp];
              ADC[1]= ftmp;
              Press1= ftmp * analog_scale[1] + analog_offset[1];
           ...
           if(itmp>= 0)
           {
              ftmp=dat_i[itmp];
              ADC[2]= ftmp;
              Press2= ftmp * analog_scale[2] + analog_offset[2];
           }
           ...
           if(itmp>= 0)
           {
              ftmp=dat_i[itmp];
              ADC[3]= ftmp;
              Temp1= ftmp * analog_scale[3] + analog_offset[3];
           }
           ...
           if(itmp>= 0)
           {
              ...
              ftmp=dat_i[itmp];
              ADC[4]= ftmp;
              Temp2= ftmp * analog_scale[4] + analog_offset[4];
           }
           ...
           if(itmp>= 0)
           {
              ftmp=dat_i[itmp];
              ADC[5]= ftmp;
              Press3= ftmp * analog_scale[5] + analog_offset[5];
           }
           ...
           if(itmp>= 0)
           {
              ftmp=dat_i[itmp];
              ADC[6]= ftmp;
              Temp3= ftmp * analog_scale[6] + analog_offset[6];
           }
       }
/*=============
