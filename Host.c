...
int t_cost_v=-1;
float cost_v1=-1;       // Unit price
float cost_v=-1;       // Unit price
float cost_v2=-1;       // Unit price
...
float dsc_tmp=0;
float offs_cor=0;
float fct_offs=0.50,fct_delay=0;
float fct_offs_r=0.50;
...
float dens_cutoff=1500.;   // for mass flow kg/h
float dens_cutoff1=3.;     // for volume flow  l/s
//----------------------
...
//----------------------
float ftmp_grk1,ftmp_grk2;
struct dis_set  ds_grk[]=
{
  &ftmp_grk1,
  -BIG_P,
  BIG_P,
...
T_INT,        // 11   i7060_out[0],
T_INT,        // 12   i7060_inp[0],
...
T_FLOAT,      // 16   VolT_id1
T_FLOAT,      // 17   VolT_id1
T_FLOAT,      // 18   summ_v_id1

T_FLOAT,      // 19  s_MVD[1].FlowM
T_FLOAT,      // 20  s_MVD[1].FlowV
T_FLOAT,      // 21  s_MVD[1].MassI
T_FLOAT,      // 22  s_MVD[1].VolI
T_FLOAT,      // 23  s_MVD[1].MassT
T_FLOAT,      // 24  s_MVD[1].VolT
T_FLOAT,      // 25  s_MVD[1].Dens
T_FLOAT,      // 26  s_MVD[1].Temp

T_FLOAT,      // 27  s_MVD[0].PressD
T_FLOAT,      // 28  s_MVD[0].PressB
T_FLOAT,      // 29  s_MVD[0].PressG
T_FLOAT,      // 30  s_MVD[0].VolI

T_INT,        // 31  test_int           // 31
T_INT_L,      // 32  int test_lng       // 32
T_INT_L,      // 33  int test_lng1      // 33
T_FLOAT,      // 34  test_float         // 34
...
/*
  vol_p   = TSD_value[0][0];
  amount_p= TSD_value[0][1];
*/

  vol_p   = TSD_value[0][1];
  amount_p= TSD_value[0][0];
...
//---------------------------------
        ...
        {   //'DRV'
         ...
         {
           if(i > icp_lst_max) i=icp_lst_max;
...
//---------------------------------
       if (!strcmp(intrpr.wrd,"P" ))
        {   //'P ' - вывести измеренное время
          flag_prn ^= 1;
          fl_pr_pmln ^= 1;
          printf(" %d %d",flag_prn,fl_pr_pmln);
          io_1=-1;
          goto fin;
        }
//---------------------------------
       ...
       {   //'VL'  Variable Load
         ...
         ee_nn1=0;
          for(i=0;(i<(i1-4)) && (i<max_buf_eep);i++)
          {
            ch_tmp=getch();
            *ch_ptr++ =ch_tmp;
            ee_nn1 +=((unsigned long)(ch_tmp)) & 0xffL;
          }
          ch_ptr=(char *)&ee_nn;
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"CTR" ))
        {   //'CTR'
          f_dis_set(ds_ctr,3,3);
          goto fin;
        }
//---------------------------------
/*
      if (!strcmp(intrpr.wrd,"TSDS" ))
        {   //'TSDS'

          if(f_dis_set(&ds_tsds,4,1) > 0)
          {
           strcpy( TsdBuf[0][0] , str_tsds);
          }
          goto fin;
        }
*/
...
//---------------------------------
#if defined(RTU_Lnk)
      if (!strcmp(intrpr.wrd,"LIN" ))
        {   //'LIN'
          f_dis_set(ds_lin,3,4);
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"LINT" ))
        {   //'LINT'
          f_prep_state();
          f_dis_set(ds_lint,4,4);
          goto fin;
        }
//---------------------------------
#endif
...
//---------------------------------
      ...
      if (!strcmp(intrpr.wrd,"IO2" ))
        {   //'IO2'
          f_dis_set(ds_io2,3,4);
          goto fin;
        }
*/
//---------------------------------
      if (!strcmp(intrpr.wrd,"IOX" ))
        {   //'IOX'
          printf(" %d %d",GetDi1(),GetDi2());
          goto fin;
        }
//---------------------------------
      if (!strcmp(intrpr.wrd,"IOXW" ))
        {   //'1 '

        par1=GetDo1();
        par2=GetDo2();
           f_dis_set(ds_par,4,2);
        SetDo1((int)par1);
        SetDo2((int)par2);
        /*
        ...
        */
/*
           sprintf(buf_tmp," %05ld ",(long int)(par1 * 100.+0.5) );
           puts(buf_tmp);
           sprintf(buf_tmp," %06ld ",(long int)(par2+0.5) );
           puts(buf_tmp);
           sprintf(buf_tmp," %04d ",(int)(par3+0.5) );
           puts(buf_tmp);
*/
          goto fin;
        }
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"IOD" ))
        {   //'IOD'
          f_dis_set(ds_iod1,3,4);
          goto fin;
        }
//---------------------------------
      if (!strcmp(intrpr.wrd,"DPD" ))
        {   //'DPD' Display Process Data
          f_dis_set(ds_dpd,3,6);
          goto fin;
        }
//---------------------------------
/*
      if (!strcmp(intrpr.wrd,"GRK" ))
        {   //'GRK '
         ftmp_grk1=-(s_MVD[0].VolI +Vol_kd );
         ftmp_grk2=-(s_MVD[0].MassI+Mass_kd);
         if(f_dis_set(ds_grk,3,2)>0)
         {
          if((ftmp_grk1==0.) && (s_MVD[0].VolI==0.)) Vol_kd=0.;
          else
            Vol_kd  = (ftmp_grk1*(-1.0)) + s_MVD[0].VolI*(-1.0);

          if((ftmp_grk2==0.) && (s_MVD[0].MassI==0.)) Mass_kd=0.;
          else
            Mass_kd=  (-ftmp_grk2) - s_MVD[0].MassI;
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"GRKW" ))
        {   //'GRKW '
           ...
           if(Mass_kd != ftmp_grk1) goto fin_e;
           if(Vol_kd != ftmp_grk2)  goto fin_e;
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"MVP3" ))
        {   //'MVP3' Display MVD3 parameters
          ii_MVD=2;
          nn_mvd=2;
          goto m_mvp;
        }
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"MVE3" ))
        {   //'MVE3' Display MVD2 Error registers
          ii_MVD=2;
          nn_mvd=2;
          goto m_mve;
        }
      ...
      if (!strcmp(intrpr.wrd,"VOL" ))
        {   //'VOL' VOL
         if( f_dis_set(ds_vol,3,1) > 0)
         {
          ...
          if(cost_v <= 0) goto m_wrg;
          if(FL_err)   goto m_wrg;
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"VOLL" ))
        {   //'VOLL' VOLL
          f_dis_set(ds_vol,4,1);
          goto fin;
        }
//---------------------------------
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"MVZ3" ))
        {   //'MVZ2'
          if(f_reset_zer_MVD(2) == 0) goto fin_e;
          goto fin;
        }
//---------------------------------
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"NA" ))
        {   //'NA'
          ...
          ftmp_na[4]=analog_scale[4]*NA_scale;
          ftmp_na[5]=analog_scale[5]*NA_scale;
          ftmp_na[6]=analog_scale[6]*NA_scale;

          if(f_dis_set(ds_na,2,7)>0)
          {
           ...
           analog_scale[4]=ftmp_na[4]/NA_scale;
           analog_scale[5]=ftmp_na[5]/NA_scale;
           analog_scale[6]=ftmp_na[6]/NA_scale;
          }
          goto fin;
        }
//---------------------------------
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"IEN" ))
        {   //'IEN'
          f_dis_set(&ds_ien,3,1);
          if(en_mski != 0) en_mski=0xff;
          goto fin;
        }
//---------------------------------
      if (!strcmp(intrpr.wrd,"RND" ))
        {   //'RND'
          f_dis_set(ds_rnd,3,3);
          goto fin;
        }
//---------------------------------
      if (!strcmp(intrpr.wrd,"RNDM" ))
        {   //'RNDM'
          f_dis_set(ds_rndm,4,3);
          goto fin;
        }
//---------------------------------
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"FLD" ))
        {   //'FLD'
        printf("\n\rCurrent FS=%04X,Offset=%4X",flash_seg,flash_offset);
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"SPM" ))
        {   //'SPM'
         pdn_tmp= pmln_b_lgth;
         i2=sizeof(struct stor_buf);
...
//------------------------------------------
      if (!strcmp(intrpr.wrd,"DN" ))
        {   //'DN'
          if(sw_dns>0) ftmp_p[0]=1;
          else ftmp_p[0]=0;
          if(f_dis_set(ds_lp,2,1) >0)
          {
            if(ftmp_p[0] > 0)
            {// старт измерения плотности
                if(sw_dns>0) goto fin_e;
  // старт      измерения плотности
                sw_mmi=210;
                sw_dns=1;
            }
            else if(ftmp_p[0] == 0)
            {// стоп измерения плотности
                if(sw_dns <= 0) goto fin_e;
              // стоп измерения плотности
                sw_mmi=211;
                sw_dns=12;
...
//------------------------------------------
      if (!strcmp(intrpr.wrd,"DNP" ))
        {   //'DNP' permissible values of density
          f_dis_set(ds_dnp,3,4);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"DNM" ))
        {   //'DNM' режим включени прокачки
    if(MVD_FLOW != 0) MVD_FLOW_t=1;
    else  MVD_FLOW_t=3;
          f_dis_set(ds_dnm,3,5);
...
//------------------------------------------
      if (!strcmp(intrpr.wrd,"DNO" ))
        {   //'DNO' parameters for check Dens switch is off
          f_dis_set(ds_dno,3,5);
          goto fin;
        }
//------------------------------------------
...
//------------------------------------------
      if (!strcmp(intrpr.wrd,"AGP" ))
        {   //'AGP'  - отладочная директива с состояниями для f_pump_run
          f_dis_set(ds_agp,3,5);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"PART" ))
        {   //'PART' PARameters of Time
          f_dis_set(ds_part,4,6);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"PARTR" ))
        {   //'PART' PARameters of Time
          f_dis_set(ds_partr,5,3);
          goto fin;
        }
//------------------------------------------
...
//------------------------------------------
/*
      if (!strcmp(intrpr.wrd,"KM" ))
        {   //'KM'
          f_dis_set(ds_km,2,3);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"KM2" ))
        {   //'KM2'
          f_dis_set(ds_km2,2,3);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"KM3" ))
        {   //'KM3'
          f_dis_set(ds_km3,2,3);
          goto fin;
        }
*/
//------------------------------------------
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"PARPR" ))
        {   //'PARPR'
          f_dis_set(ds_parpr,5,4);
          goto fin;
        }
//---------------------------------
//------------------------------------------
...
//------------------------------------------
 ...
 printf("\n\r WMC 57 1 -- Reset volume total");
         */
 printf("\n\r MVZ  -- Zeroing of MVD");
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"EX" ))
        {   // 'EX' Max недолив, перелив
          f_dis_set(ds_ex,2,3);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"FLCH" ))
        {   //'FLCH'FLow CHeck
          f_dis_set(ds_flch,4,6);
          goto fin;
        }
//------------------------------------------
      if (!strcmp(intrpr.wrd,"FLCHR" ))
        {   //'FLCHR'FLow CHeck for receive
          f_dis_set(ds_flchr,5,4);
          goto fin;
        }
//------------------------------------------
...
//  if(itmp & Gun_off )
    ...
    if((itmp & Hst_all)==Hst_alm)
      printf("\nАвария от HOST ");
    if((itmp & Hst_all)==Gun_off)
       printf("\r\nНасос отключен при наливе");
    if((itmp & Hst_all)==MotNotStop)
       printf("\r\nПриемный насос не отключился");
  }
...
/*
  if(i2==0)
       printf("\r\n Ошибок нет");
*/
   printf("\r\n sw_dns = %d, sw_mmi=%d,sw_dlv_liq=%d " ,sw_dns,sw_mmi,sw_dlv_liq);
   printf("\r\n flag_rcv = %d, i_dens=%4x,fl_GO_rcv=%d,fl_GO = %d",flag_rcv,i_dens,fl_GO_rcv,fl_GO );
   printf("\r\n stage_max = %d,st_MVD_sw =%d,ff_serv_N=%8x ",stage_max,st_MVD_sw,(int)ff_serv_N );
   printf("\r\n MVD_fl[0]=%d,MVD_fl[1]=%d,cl_MVD_sw=%d",MVD_fl[0],MVD_fl[1],cl_MVD_sw );
/*
   if(sw_dns> 0)
   {
       puts("\r\n Измерение плотности:");
       puts("\r\nПлотность  : ");
       printf("%8.2f кг/м3   ",s_MVD[0].Dens);
       puts("\r\nТемпература: ");
       printf(" %7.2f град.   ",s_MVD[0].Temp);
       puts("\r\nДавление   : ");
       printf(" %7.3f МПа     ",s_MVD[0].Press-0.1);
   }
   if(fl_GO != 0)
    {
     if(vol_f != BIG_P)
...
void   f_menu()
{
 printf("\n\rREP - отчет о состоянии системы");
}
//-------------------------------
...
{
 union  { float f; unsigned long l; } o;
  // записывает значения Mass_kd , Vol_kd в расходомер

//  if((MVD_fn[0] != 0) && (MVD_fn[0] != F_FAULT))
  if(MVD_fn[0] != 0)
  {
      if( f_timer(t_kd_wr,(long int)1000L ) == 0 ) return;
      kd_sw=-1;
      ff_serv=NULL;
      printf("\n\r Err WR MVD");
      return;
  ...
  switch (kd_sw)
   {
    case 1:
         ...
         MVD_fn[0]=MVD_WR_F;
         s_MVD[0].r_addr = Addr_Mass_kd; // 241
         s_MVD[0].f= Mass_kd;
         t_kd_wr=TimeStamp;
         kd_sw=2;
         break;
    ...
    case 2:
         MVD_fn[0]=MVD_WR_F;
         s_MVD[0].r_addr = Addr_Vol_kd;  // 243
  //     printf("\r\n Vol_kd=%f",Vol_kd);
  //      o.f= Vol_kd;
  //      printf("  0x=%08lx",o.l);
    ...
    case 3:
         ...
         MVD_fn[0]=MVD_WR_I;
         s_MVD[0].r_addr = Addr_Fl_grk;  // 139
         s_MVD[0].i= Fl_grk_OK;
         t_kd_wr=TimeStamp;
         kd_sw=4;
         return;
